...
void initialize_dyn_block_system()
{
	...
	uint32 MemBlockSize=sizeof(*MemBlockNodes)*NUM_OF_UHEAP_PAGES;
	uint32 FreeSize=USER_HEAP_MAX-USER_HEAP_START;

	sys_allocate_chunk(USER_DYN_BLKS_ARRAY,MemBlockSize,PERM_WRITEABLE|PERM_USER);
...
void free(void* virtual_address)
{
	...
	uint32 block_sva=Found->sva;
	uint32 block_size=Found->size;

	//cprintf("FREE CALLED FOUND SVA: %x    FOUND SIZE:%d    REQUIRED SVA: %x \n"
		//	,Found->sva,Found->size,virtual_address);
	LIST_REMOVE(&(AllocMemBlocksList),Found);
	insert_sorted_with_merge_freeList(Found);
	sys_free_user_mem(block_sva,block_size);
}
...
void* smalloc(char *sharedVarName, uint32 size, uint8 isWritable)
{
	...
	//	2) if no suitable space found, return NULL
  if(sys_isUHeapPlacementStrategyFIRSTFIT()){
   uint32 ff_size=ROUNDUP(size,PAGE_SIZE);
	struct MemBlock *Returned=alloc_block_FF(ff_size);
	if(Returned==NULL)
			return NULL;
	insert_sorted_allocList(Returned);
	uint32 va =Returned->sva;
	...
	int re =sys_createSharedObject(sharedVarName,size,isWritable,(void*)va);
	if(re<0)
		return NULL;
	return (void*)va;
  }
	...
	if(sys_isUHeapPlacementStrategyFIRSTFIT()){
	  ...
	  if(Returned==NULL)
		 return NULL;
			insert_sorted_allocList(Returned);
			uint32 va =Returned->sva;
		int re = sys_getSharedObject(ownerEnvID,sharedVarName,(void*)va);
		if(re<0)
			return NULL;
		return (void*)va;
