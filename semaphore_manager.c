...
//	panic("waitSemaphore() is not implemented yet...!!");
	...
	int index = get_semaphore_object_ID(ownerEnvID, semaphoreName);
	semaphores[index].value=semaphores[index].value - 1;
...
//	panic("signalSemaphore() is not implemented yet...!!");
	...
	semaphores[index].value=semaphores[index].value+1;
	if (semaphores[index].value<=0){
		myenv = dequeue(&semaphores[index].env_queue);
