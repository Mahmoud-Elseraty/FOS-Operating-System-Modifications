...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
    ...
    struct Share *all_obj=NULL;
    int all_re = allocate_share_object(&all_obj);
    if(all_re==E_NO_SHARE)
    	return E_NO_SHARE;
    	...
    	for(int i=sva;i<eva;i+=PAGE_SIZE){
    		...
    		struct FrameInfo* tmp=get_frame_info(myenv->env_page_directory,i,&ptr_page_table);
    		if(tmp!=NULL)
    			return -1;
    	}
    	//frame allocation,mapping,adding_frame_to_storage
    	uint32 index=0;
    	for(int i=sva;i<eva;i+=PAGE_SIZE){
    		struct FrameInfo *ptr_frame_info;
    		int ret = allocate_frame(&ptr_frame_info);
    		add_frame_to_storage(all_obj->framesStorage,ptr_frame_info,index);
    		index++;
    		ret=map_frame(myenv->env_page_directory,ptr_frame_info,i,PERM_WRITEABLE|PERM_USER|PERM_USED|PERM_PRESENT);
    		ptr_frame_info->va=i;
    	}
    	all_obj->ownerID=ownerID;
    	strcpy(all_obj->name,shareName);
    	all_obj->size=size;
    	all_obj->references=1;
    	all_obj->isWritable=isWritable;
        int obj_idd =get_share_object_ID(ownerID,shareName);
	    if(obj_idd==E_SHARED_MEM_NOT_EXISTS)
	    	return E_SHARED_MEM_NOT_EXISTS;
	    else
	    	return obj_idd;
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	int index =0;
	for(int i=va;i<(va+size);i+=PAGE_SIZE)
	{
		  struct  FrameInfo *ptr=get_frame_from_storage(shares[obj_id].framesStorage,index);
		  if(shares[obj_id].isWritable==1)
			  map_frame(myenv->env_page_directory,ptr,i,PERM_WRITEABLE|PERM_USER|PERM_USED|PERM_PRESENT);
		  else
			  map_frame(myenv->env_page_directory,ptr,i,PERM_USER|PERM_USED|PERM_PRESENT);
		  index++;
	}
